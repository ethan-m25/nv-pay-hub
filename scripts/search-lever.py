#!/usr/bin/env python3
"""
nv-pay-hub/scripts/search-lever.py
Lever job board scraper — Nevada edition.
Nevada NRS 613.133 (AB309, eff. Oct 2023) — all employers must include pay range in all job postings and promotions
Run: python3 ~/nv-pay-hub/scripts/search-lever.py
"""
...
LOG_FILE  = os.path.expanduser("~/nv-pay-hub/scripts/lever.log")
LOCK_FILE = os.path.expanduser("~/nv-pay-hub/scripts/.lever.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"

log = make_logger(LOG_FILE)
fetcher = Fetcher()

SEED_SLUGS = ['3e', 'allegiant', 'switch']
SEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))

DISCOVERY_QUERIES = ['site:jobs.lever.co "Las Vegas" OR "Nevada" OR "Reno" salary 2025 2026', 'site:jobs.lever.co "Las Vegas, NV" OR "Henderson, NV" engineer manager', 'site:jobs.lever.co "Nevada" pay range engineer analyst']
...
def main():
    ...
    log(f"\n=== Lever scraper complete: {total_found} new NV jobs (api_failures={api_failures}) ===")
    return 0
