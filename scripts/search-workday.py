#!/usr/bin/env python3
"""
nv-pay-hub/scripts/search-workday.py
Workday API scraper — Nevada edition.

Nevada Pay Transparency (NRS 613.133, effective Oct 1, 2021)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Nevada employers
  2. CXS JSON API   — paginate all jobs per employer, filter Nevada (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/nv-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/nv-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/nv-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Nevada"
...
# ── Seed tenants ─────────────────────────────────────────────────────────────
SEED_TENANTS = [
    ("mgmresorts.wd5.myworkdayjobs.com", "mgmresorts", "MGMCareers", "MGM Resorts International"),
    ("caesars.wd5.myworkdayjobs.com", "caesars", "External", "Caesars Entertainment"),
...
KNOWN_COMPANY_OVERRIDES = {
    "mgmresorts": "MGM Resorts International",
    "caesars": "Caesars Entertainment",
}

DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Nevada" salary 2026',
    'site:myworkdayjobs.com "Las Vegas" salary "$" 2026',
    'site:myworkdayjobs.com "Reno" OR "Henderson" OR "North Las Vegas" salary 2026',
    'site:myworkdayjobs.com Nevada gaming OR hospitality OR tech salary 2026',
]
...
def is_nevada(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Nevada.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_nevada(p.get("locationsText", ""), p.get("externalPath", "")):
                    nv_jobs.append(p)
            offset += limit
        ...
        log(f"  Nevada jobs: {len(nv_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(nv_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_nevada(location, external_path):
            continue
