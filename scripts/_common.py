#!/usr/bin/env python3
"""
nv-pay-hub/scripts/_common.py
Shared utilities for Nevada Pay Hub search scripts.
Nevada NRS 613.133 (AB309, eff. Oct 2023) — all employers must include pay range in all job postings and promotions
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"nv-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/nv-pay-hub/data/jobs.json")
CURRENCY    = "USD"
REGION      = "NV"
...
REGION_TERMS = ['nevada', 'las vegas', 'henderson', 'reno', 'sparks', 'north las vegas', 'paradise', 'enterprise', 'spring valley', 'summerlin', 'boulder city', 'elko', ', nv,', 'nv 89']

_NON_REGION_LOC_TERMS = ['portland, or', 'eugene, or', 'salem, or', 'oregon', 'washington, dc', 'washington dc', 'district of columbia', 'seattle, wa', 'bellevue, wa', 'denver, co', 'boulder, co', 'chicago, il', 'new york city', 'new york, ny', 'san francisco, ca', 'los angeles, ca', 'boston, ma', 'minneapolis, mn', 'honolulu, hi', 'hawaii', 'toronto', 'vancouver', 'montreal']
...
def load_existing_keys():
    ...
    if not shared.exists():
        return existing
    for f in sorted(shared.glob(f"nv-jobs-raw-*.txt")):
        try:
            for line in open(f):
                line = line.strip()
                if not line:
