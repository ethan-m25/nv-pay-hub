#!/usr/bin/env python3
"""
nv-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Nevada edition.
Nevada NRS 613.133 (AB309, eff. Oct 2023) — all employers must include pay range in all job postings and promotions
Run: python3 ~/nv-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/nv-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/nv-pay-hub/scripts/.greenhouse.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"

log = make_logger(LOG_FILE)
fetcher = Fetcher()

SEED_SLUGS = [('switch', 'Switch'), ('allegiant', None), ('3e', '3E'), ('everi', 'Everi Holdings'), ('aristocrat', None), ('wynn', 'Wynn Resorts'), ('mgm', 'MGM Resorts'), ('caesars', 'Caesars Entertainment'), ('stations', 'Station Casinos'), ('lvcva', 'Las Vegas Convention Authority')]
...
def main():
    ...
    log("=== NV Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new NV jobs written to {OUTPUT_FILE} ===")
