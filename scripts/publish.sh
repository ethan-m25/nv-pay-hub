...
if git diff --cached --quiet; then
  notify_discord "ℹ️ NV Pay Hub [$TODAY]: no changes ($ACTIVE_COUNT active)"
  exit 0
fi

git commit -m "data: NV daily update $TODAY (+$NEW_TODAY new, $ACTIVE_COUNT active)"
git push origin main 2>/dev/null || true

export PATH="/Users/clawii/.npm-global/bin:$PATH"
wrangler pages deploy . --project-name nv-payhub --branch main 2>&1 | tail -3 || true
...
notify_discord "✅ NV Pay Hub updated [$TODAY]
